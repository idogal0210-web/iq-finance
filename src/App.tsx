...
import {
  ...
  ArrowRight, ArrowLeft, Home, Plus, Menu, X, Globe,
...
function Wordmark({ size = 15, showTagline = true }: WordmarkProps) {
  return (
...
function useCountUp(target: number, duration = 2000, start = 125000): number {
...
function GlassPanel({ children, style = {}, ...props }: GlassPanelProps) {
...
function ProgressBar({ ratio, glow = false, delay = "0s" }: ProgressBarProps) {
...
function TransactionRow({ icon: Icon, title, subtitle, amount, positive = false, isRtl }: TransactionRowProps) {
...
function LayoutToggle({ mode, onToggle, label }: LayoutToggleProps) {
...
function SlideMenu({ open, onClose, lang, onToggleLang }: SlideMenuProps) {
  const l = t[lang];
  const isRtl = lang === "he";
...
function GrowthTrace({ width = 420, height = 64 }: GrowthTraceProps) {
...
function AssetCard({ icon: Icon, label, name, balance, change, positive, isRtl }: AssetCardProps) {
  const [h, setH] = useState<boolean>(false);
  return (
    <div
      onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
...
function BrushedMetalCard({ last4, type, isActive = false }: BrushedMetalCardProps) {
  const [h, setH] = useState<boolean>(false);
  return (
    <div
      onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
...
function WalletsScreen({ lang, layoutMode, onBack }: WalletsScreenProps) {
  const l = t[lang];
  const isRtl = lang === "he";
